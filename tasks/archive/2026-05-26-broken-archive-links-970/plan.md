...
**Spec:** SPEC.md — not archived with this cycle
...
**Pattern A (15 files): Replace `SPEC.md — not archived with this cycle` with `_(archived)_`.** Search-and-replace via `sed -i` or per-file Edit.
...
Exact pattern to look for: any link with text `../SPEC.md` or `Spec (not archived with this cycle)` etc. The line typically reads `**Spec:** SPEC.md — not archived with this cycle` — verify per-file.
