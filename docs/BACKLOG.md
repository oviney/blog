...
| P1 | Add the real gates to branch protection. `main` requires only `build` and `🔒 Security Audit`. The 🖼️ Visual Regression gate (Gap A) and the three 🎭 Playwright shards now fail correctly but are **not required checks**, so branch protection does not stop a merge on a red one. Command: `gh api -X PATCH repos/oviney/blog/branches/main/protection/required_status_checks -f 'contexts[]=build' -f 'contexts[]=🔒 Security Audit' -f 'contexts[]=🖼️ Visual Regression' -f 'contexts[]=🎭 Playwright Shard 1/3' -f 'contexts[]=🎭 Playwright Shard 2/3' -f 'contexts[]=🎭 Playwright Shard 3/3'`. Verify names match exactly — a typo deadlocks the queue. | repo admin | Not started — needs owner decision | — |
