...
- **Owner decision outstanding: [#1242](https://github.com/oviney/blog/pull/1242)**
  ...
  owner-only.
- **Top real P3s:** internal docs are publishing to production — `tasks/` **and**
  eight root files including `CLAUDE.md` and `AGENTS.md`, all `200 text/markdown`
  (re-measured 2026-08-15; the row previously named only `tasks/`) — owner
  decision, `_config.yml` is protected. Then `.featured-post` dead CSS and the
...
| P3 | **Retire the `.home-intro-links` name and fix the margin it zeroes.** `blog/index.html:13` stacks `class="econ-topic-browse home-intro-links"`. Both are single-class selectors (specificity 0,1,0), so source order wins and `.home-intro-links { margin: 0 }` (`economist-theme.scss:2160`) silently cancels `.econ-topic-browse`'s `margin: 0 0 $spacing-lg 0` (`:1746`) — the nav loses its intended 2rem bottom margin on all five paginated `/blog/` pages. Pre-existing, not introduced by #1203. Fix: fold the `a {}` block into `.econ-topic-browse`, drop the stray `margin: 0`, remove the class from `blog/index.html`. **This moves pixels** — expect the four `blog-index`/listing baselines to need a reseed. | `agent:creative-director` | Not started | — |
...
| P3 | ADR: adopt / defer / hybrid on `.github/agents/*.agent.md` native custom agents vs `agent:*` label routing. Decision only — avoid two competing routing systems. | `governance-update` | **Done** — ADR-010, PR #1278 | [#1110](https://github.com/oviney/blog/issues/1110) |
...
| PR-gate P1 **Stream D** — slim the PR gate | [#1197](https://github.com/oviney/blog/pull/1197) merged — `🏗️ Build Site` builds once; `🎭 Page Contract` (15 tests, Mobile + Desktop) and `🖼️ Visual Regression` judge that one artifact. Three Playwright shards, the pa11y/Lighthouse job, the quality report and change-based selection all gone from the PR path; full suite, pa11y and Lighthouse moved to `🌙 Nightly Full Suite`, which opens one deduped issue on failure. Selection was **deleted rather than repaired** — its greps never selected `@REQ-VISUAL-SNAP`, `@REQ-PAGE-CHROME`, `@REQ-CONTENT-03/04` or `@REQ-CONTENT-CAPTION-CASE`. Branch protection swapped to `build` / `🔒 Security Audit` / `🖼️ Visual Regression` / `🎭 Page Contract`, verified by throwaway PR #1199 (all four reported; no deadlock). | 2026-08-02 |
